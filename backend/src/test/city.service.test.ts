...
describe("city.service", () => {
  ...
  describe("updateAirport", () => {
    it("should throw NotUniqueError", () => {
      try {
        cityService.updateAirportByCodeAndCityID(1, "SVO", {
          name: mockAirports[0].name,
        });
      } catch (err) {
        expect(err).toBeInstanceOf(NotUniqueError);
      }
    ...
    it("should throw RelatedDataError", () => {
      try {
        cityService.updateAirportByCodeAndCityID(999, "SVO", {
          name: mockAirports[0].name,
        });
      } catch (err) {
        expect(err).toBeInstanceOf(RelatedDataError);
    ...
    it("should throw NotFoundError", () => {
      try {
        cityService.updateAirportByCodeAndCityID(1, "UUI", {
          name: mockAirports[0].name,
        });
      } catch (err) {
        expect(err).toBeInstanceOf(NotFoundError);
